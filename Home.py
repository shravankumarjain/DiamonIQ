...
with st.spinner(
    "Loading season roster from Baseball Savant — first load only, ~10-20 s…"
):
    roster = load_roster()
...
with st.sidebar:
    ...
    if roster.empty:
        ...
        name_col = (
            "player_name" if "player_name" in roster.columns else roster.columns[0]
        )
        ...
        selected_name = st.selectbox(
            ...
            index=pitcher_names.index(default_pitcher)
            if default_pitcher in pitcher_names
            else 0,
        ...
        if era != "—":
            _stat_line("ERA", era)
        if ip != "—":
            _stat_line("Innings pitched", ip)
        _stat_line("Batters faced", pa)
        _stat_line("xwOBA against", xwoba)
        if ev != "—":
            _stat_line("Avg exit velo", ev, "mph")
        st.markdown(
    ...
    st.session_state["selected_pitcher_mlbam"] = (
        int(selected_row.get("mlbam_id", 0)) if selected_row else 0
    )
...
if selected_name:
    ...
    parts = [
        f'<span style="font-size:1.0rem;font-weight:700;color:{GOLD};">⚾ {selected_name}</span>'
    ]
    sep = f'<span style="color:{BORDER};margin:0 8px;">·</span>'
    if era2 != "—":
        parts.append(
            f'{sep}<span style="font-size:0.80rem;color:{TEXT_SEC};"><b style="color:{TEXT_PRI};">{era2}</b> ERA</span>'
        )
    parts.append(
        f'{sep}<span style="font-size:0.80rem;color:{TEXT_SEC};"><b style="color:{TEXT_PRI};">{xwoba2}</b> xwOBA vs</span>'
    )
    if ev2 != "—":
        parts.append(
            f'{sep}<span style="font-size:0.80rem;color:{TEXT_SEC};"><b style="color:{TEXT_PRI};">{ev2}</b> mph EV</span>'
        )
    if pa2 != "—":
        parts.append(
            f'{sep}<span style="font-size:0.80rem;color:{TEXT_SEC};"><b style="color:{TEXT_PRI};">{pa2}</b> BF</span>'
        )
    parts.append(
        f'{sep}<span style="font-size:0.74rem;color:{TEXT_MUT};">MLBAM {mlbam2}</span>'
    )
...
modules = [
    (
        ...
        "Percentile radar against every qualified MLB pitcher this season.",
        BLUE_ACC,
    ),
    (
        "2",
        "Arsenal Profile",
        "Is his mechanics where it should be?",
        "Movement shape, spin rate, and release-point drift, start to start.",
        BLUE_ACC,
    ),
    (
        "3",
        ...
        "Location heatmaps split by pitch type, count, and batter handedness.",
        BLUE_ACC,
    ),
    (
        "4",
        "Pitch Effectiveness",
        "Which pitches are actually working?",
        "Run Value per 100 pitches, CSW%, and xwOBA trend across the season.",
        GREEN,
    ),
    (
        "5",
        "Pitch Sequencing",
        "Is he setting hitters up, or predictable?",
        "Markov transition matrix showing what pitch follows what, and how exploitable that pattern is.",
        GREEN,
    ),
    (
        "6",
        "Workload & Fatigue",
        "Should he start, and when do we pull him?",
        "ACWR injury-risk ratio, inning-by-inning velocity decay, and a clear start/pull verdict.",
        AMBER,
    ),
    (
        "7",
        "Stuff Grade Model",
        "How good is each pitch on a scout's scale?",
        "Machine-learning model scoring every pitch 20 to 80, same scale scouts use.",
        AMBER,
    ),
    (
        "8",
        "Game Planning Engine",
        "How do we attack tonight's entire lineup?",
        "Prescriptive script per batter, a Monte Carlo at-bat simulator, and a full 9-batter scouting report.",
        RED,
    ),
]
...
steps = [
    (
        ...
        BLUE_ACC,
        "Select a pitcher",
        "Sidebar dropdown any MLB pitcher from the 2024 season.",
    ),
    (
        "2",
        GREEN,
        "Check readiness",
        "Page 6 gives an immediate start/pull recommendation.",
    ),
    (
        "3",
        GOLD,
        "Study the arsenal",
        "Pages 1–5 cover ranking, mechanics, location, effectiveness, sequencing.",
    ),
    (
        "4",
        RED,
        "Build the game plan",
        "Page 8 generates the full lineup scouting report before first pitch.",
    ),
]
...
with col_g1:
    st.markdown(
        f'<div style="font-size:0.82rem;font-weight:700;color:{TEXT_PRI};margin-bottom:8px;">Metric abbreviations</div>',
        unsafe_allow_html=True,
    )
...
with col_g2:
    st.markdown(
        f'<div style="font-size:0.82rem;font-weight:700;color:{TEXT_PRI};margin-bottom:8px;">Pitch type codes</div>',
        unsafe_allow_html=True,
    )
