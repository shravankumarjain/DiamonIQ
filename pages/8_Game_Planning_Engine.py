...
from src.components import (
    ...
    inject_css, page_header, coaches_insight, apply_theme, metric_row, rgba,
    BG_CARD, BG_CARD2, BORDER,
    NAVY, GOLD, BLUE_ACC, GREEN, RED, AMBER,
    TEXT_PRI, TEXT_SEC, TEXT_MUT,
    pitch_color, pitch_effectiveness,
)
...
def load_batters():
    try:
        ...
        for c in ["estimated_woba_using_speedangle", "xwoba", "est_woba", "xwoba_using_speedangle"]:
            if c in df.columns:
...
def simulate_at_bats(eff_df, xwoba_b_val, pitch_weights=None, n_sim=1000, seed=42):
    ...
    for _, row in eff_df.iterrows():
        ...
        pitch_probs[pt] = {
            "whiff_p": whiff_p_clamped, "called_p": called_p,
            "ball_p": ball_p, "contact_p": contact_p, "rv": rv,
        }
    ...
    if w_list:
        w_sum = sum(w_list)
        w_list = [w / w_sum for w in w_list] if w_sum > 0 else [1.0 / len(w_list)] * len(w_list)
    ...
    for _ in range(n_sim):
        ...
        for _ in range(12):
            ...
            pp = pitch_probs.get(
                chosen_pt,
                {"whiff_p": 0.25, "called_p": 0.10, "ball_p": 0.35, "contact_p": 0.30, "rv": 0.0},
            )
            roll = rng.random()
            total_rv += pp["rv"] / 100
...
with st.sidebar:
    ...
    if not batter_df.empty and "player_name" in batter_df.columns:
        ...
        default_b = next(
            (b for b in ["Freddie Freeman", "Juan Soto", "Mookie Betts",
                          "Austin Riley", "Jose Ramirez", "Rafael Devers"]
...
with col_b:
    if selected_batter and not batter_df.empty:
        ...
        if not brow_df.empty:
            ...
            threat     = "HIGH" if xwoba_b >= 0.360 else "MEDIUM" if xwoba_b >= 0.310 else "LOW"
            threat_col = RED if threat == "HIGH" else AMBER if threat == "MEDIUM" else GREEN
            st.markdown(
...
if "plate_x" in df_pitcher.columns and "plate_z" in df_pitcher.columns:
    ...
    def zone_grid(data, outcome_col, title, invert=False):
        ...
        if len(valid) < 20:
            return None
        valid["xb"] = pd.cut(valid["plate_x"].clip(-0.83, 0.83),
                             bins=[-0.83, -0.277, 0.277, 0.83],
                             labels=["In", "Mid", "Out"])
        valid["zb"] = pd.cut(valid["plate_z"].clip(1.5, 3.5),
        ...
        try:
            pivot = pivot.reindex(index=["High", "Mid", "Low"], columns=["In", "Mid", "Out"])
        except Exception:
            pass
        if pivot.empty:
            return None
        cscale = (
            [[0, "rgba(231,76,60,0.9)"], [0.5, "rgba(243,156,18,0.6)"], [1, "rgba(46,204,113,0.9)"]]
            if invert
            else [[0, "rgba(46,204,113,0.9)"], [0.5, "rgba(243,156,18,0.6)"], [1, "rgba(231,76,60,0.9)"]]
        )
        ...
        fig.add_shape(type="rect", x0=-0.5, x1=2.5, y0=-0.5, y1=2.5,
    ...
    with col_z2:
        if "in_zone" in df_hand.columns and "is_swing" in df_hand.columns:
            ...
            fig_c = zone_grid(ooz, "is_swing", f"Chase% out-of-zone — vs {batter_hand}HB", invert=True)
            if fig_c:
...
if selected_batter and not batter_df.empty:
    ...
    if batter_hand == "R":
        ...
        putaway_zone = "Bury the breaking ball below the zone or elevate the fastball against LHB."
    else:
    ...
    if threat == "HIGH":
        ...
        threat_note = "Attack early in counts and be aggressive. Do not nibble at the edges."
    ...
    for border_col, label_col, section_label, body in [
        (NAVY,  NAVY,  "FIRST PITCH APPROACH",
    ...
    st.dataframe(
        pd.DataFrame({
            "Count":             ["0-0", "0-1 or 1-1", "0-2 or 1-2", "2-2 or 3-2", "3-0 or 3-1"],
            "Recommended pitch": [lead_pitch, f"{lead_pitch} or change speed", putaway,
                                  putaway, "Fastball for strike"],
            "Intent":            ["Take count lead", "Set up putaway pitch via tunneling",
                                  "Strikeout attempt", "Execute — no walks",
                                  "Get strike — protect zone"],
        }),
        use_container_width=True, hide_index=True,
    )
else:
    st.info("Select an opposing batter in the sidebar to generate the prescriptive game plan.", icon="🎯")
...
if selected_batter and not eff.empty and not batter_df.empty:
    ...
    for i, (_, row) in enumerate(top_pitches.iterrows()):
        ...
        with sim_cols[i]:
            pitch_weights[pt] = st.slider(
                f"{pt} %", min_value=0, max_value=80,
                value=int(float(row["usage_pct"])), step=5, key=f"sim_{pt}",
            )

    if sum(pitch_weights.values()) <= 0:
        st.warning("All pitch usage sliders are at 0%. Using equal weighting for the simulation.")

    xwoba_b_val = get_xwoba(batter_df[batter_df["player_name"] == selected_batter].iloc[0])
    outcomes, avg_rv = simulate_at_bats(eff, xwoba_b_val, pitch_weights=pitch_weights, n_sim=n_sim)

    # Result cards
    result_cols = st.columns(4)
    for col_w, (label, key, colour) in zip(result_cols, [
    ...
    avg_rv_color = GREEN if avg_rv < 0 else RED   # pre-computed — no nested f-string quotes
    st.markdown(
...
# ── Section 5: Tonight's lineup scouting report ───────────────────────────────
st.markdown("### Tonight's lineup scouting report")
st.caption(
    f"{pitcher_name} is starting tonight against a known, fixed lineup of 9 batters. "
...
if lineup_batters and not eff.empty and not batter_df.empty:
    ...
    for b_name in lineup_batters:
        ...
        b_outcomes, b_avg_rv = simulate_at_bats(eff, b_xw, n_sim=500, seed=hash(b_name) % (2**31))
    ...
    if lineup_rows:
        ...
        for col_w, (label, value, colour) in zip(sum_cols, [
        ...
        lineup_df = lineup_df.sort_values("_sort").drop(columns="_sort").reset_index(drop=True)
    ...
    st.info(
        "No lineup entered yet. Look up tonight's opponent and add their announced "
        "batting order in the sidebar under **Tonight's lineup**. Scroll Down The Left Panel The report builds "
...
st.caption("Rank the opposing lineup by threat level. Prioritise game-plan time on HIGH-threat matchups.")

if not batter_df.empty and "player_name" in batter_df.columns:
    with st.expander("View full batter leaderboard"):
        xwoba_col = next(
            (c for c in ["xwOBA", "xwoba", "estimated_woba_using_speedangle", "est_woba"]
        ...
        if xwoba_col:
            ...
            extra_cols = [c for c in ["AvgEV", "Barrel_pct", "HardHit_pct", "pa"] if c in ldb.columns]
            st.dataframe(
                ldb[show_cols + extra_cols].head(60).rename(columns={"player_name": "Batter", xwoba_col: "xwOBA"}),
                use_container_width=True, hide_index=True,
            )
        else:
            st.dataframe(batter_df[[c for c in batter_df.columns if c != "mlbam_id"][:8]].head(30),
                         use_container_width=True, hide_index=True)
else:
    st.info("Batter data not loaded. Run `python scripts/01_download_base_data.py` first.")
...
if selected_batter and not batter_df.empty:
    ...
    insight = (
        ...
        + (f"The simulation projects a {so_pct:.0f}% strikeout rate with the current pitch mix." if so_pct > 0 else "")
    )
    action = (
